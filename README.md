...
2. **Greeting** — two parts, only when stage 1 fires: a short spoken **hello**
   rendered from one of five rotating templates (string substitution, no model),
   plus a **question card** generated by **Qwen3.5** (`0.8B` / `2B` / `9B`, GGUF from
   [`unsloth`](https://huggingface.co/unsloth)) via **llama-cpp-python**.
...
# Download models once: face ONNX (~2.5 MB) + the Qwen3.5 GGUFs
...
uv run python main.py --model 2B      # larger model (or --model 9B)
...
| `greeter.py`  | templated hello (Part 1) + Qwen3.5 question card (Part 2, lazy-loaded) |
...
rotation of single-acrostic **SLOP** cards (`pool/SLOP/`). This is a **separate pool**
from the main `SLOP/TIAT/LEEB` cards (`pool/SLOP+TIAT+LEEB/`): the background producer
fills both independently — never mixing them — balancing its runs so neither starves,
with the two pools sharing only the one loaded model.
